...
Course correction (2026-08-18, Levi): the app should LOOK like the predecessor, not merely rhyme with it. When building or fixing UI, port the actual patterns (layout, spacing, tokens, component shapes) from `levi-sheet/draftroom/app.html` rather than redesigning from memory. The infrastructure is new; the look is inherited. Formalized 2026-08-19 into [UI-CARRYOVER.md](UI-CARRYOVER.md): the component-by-component harvest inventory (audited against the original at V36), bucketed carry-as-is / adapt / cannot-carry, with the round plan. That document governs all remaining Phase 4 UI work.
